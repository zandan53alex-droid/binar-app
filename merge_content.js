...
const extractObjectString = (filepath) => {
    const content = fs.readFileSync(filepath, 'utf-8');
    const start = content.indexOf('{');
    const end = content.lastIndexOf('};');
    return content.substring(start, end !== -1 ? end + 1 : content.lastIndexOf('}') + 1);
};

const enStr = extractObjectString('c:/binarny/webapp/content_en.js');
const esStr = extractObjectString('c:/binarny/webapp/content_es.js');
const frStr = extractObjectString('c:/binarny/webapp/content_fr.js');
const hiStr = extractObjectString('c:/binarny/webapp/content_hi.js');

const ruCoursesStr = fs.readFileSync('c:/binarny/webapp/courses_orig.js', 'utf-8');
const ruLessonsStr = fs.readFileSync('c:/binarny/webapp/lessons_orig.js', 'utf-8');
...
const ruStr = \`{
    COURSES: \${ruCoursesStr},
    LESSONS: \${ruLessonsStr},
    BOOKS: \${ruBooksStr},
    INDICATORS: \${ruIndicatorsStr}
}\`;

const finalContent = \`const MARKET_CONTENT = {
    ru: \${ruStr},
    en: \${enStr},
    es: \${esStr},
    fr: \${frStr},
...
fs.writeFileSync('c:/binarny/webapp/content.js', finalContent, 'utf-8');
console.log("content.js built successfully with all 5 languages and full text!");
