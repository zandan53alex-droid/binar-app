...
def extract_array(filepath, array_name):
    ...
    # Robust parser that ignores brackets inside strings
    # and handles both 'array_name: [' and 'array_name = ['
    for pattern in [f"{array_name}: [", f"{array_name} = [", f"{array_name}:[", f"{array_name}= ["]:
        idx = content.find(pattern)
        if idx != -1:
            start_idx = idx + content[idx:].find("[")
            stack = []
            in_string = None
...
# Source for Russian Indicators from files
ru_ind_map = {
...
# Build ru_indicators manually to ensure it's exact
ru_indicators = "[\n"
...
for lang in langs:
    path = f"c:/binarny/webapp/content_{lang}.js"
    crs = extract_array(path, "COURSES")
    lsn = extract_array(path, "LESSONS")
    bks = extract_array(path, "BOOKS")
    ind = extract_array(path, "INDICATORS")
    lang_data[lang] = "{ COURSES: " + crs + ", LESSONS: " + lsn + ", BOOKS: " + bks + ", INDICATORS: " + ind + " }"
...
print("SUCCESS: content.js rebuilt V4.")
