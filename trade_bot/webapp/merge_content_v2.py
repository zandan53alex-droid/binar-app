...
def read_indicator_text(folder, file):
    ...
    if os.path.exists(path):
        with codecs.open(path, 'r', 'utf-8') as f:
            ...
            # We want actual newlines in the file, so we don't replace \n with \\n
            # but we must escape backticks
            return t.replace('`', '\\`').replace('\\', '\\\\')
    return ""

# Indicator folders/files mapping for Russian
ru_ind_map = {
...
for lang in langs:
    ...
    lang_data[lang] = f"{{ COURSES: {courses}, LESSONS: {lessons}, BOOKS: {books}, INDICATORS: {indicators} }}"
...
ru_data = f"{{ COURSES: {ru_courses}, LESSONS: {ru_lessons}, BOOKS: {ru_books}, INDICATORS: {ru_indicators} }}"

final_js = "const MARKET_CONTENT = {\n"
final_js += f"    ru: {ru_data},\n"
for l in langs:
    final_js += f"    {l}: {lang_data[l]},\n"
# Remove trailing comma if needed, but JS allows it. We'll just close it.
final_js = final_js.rstrip(',\n') + "\n};\n"

with codecs.open("c:/binarny/webapp/content.js", "w", "utf-8") as f:
    f.write(final_js)

print("SUCCESS: content.js rebuilt.")
