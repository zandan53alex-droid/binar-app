...
async def pocket_option_client():
    ...
    while True:
        ...
        try:
            ...
            # В новых версиях websockets (14+) переименовали в additional_headers, 
            # во многих старых используется extra_headers. Пробуем универсально.
            ws_major_version = 13
            try:
                # websockets.__version__ - стандартный способ
                ver_str = getattr(websockets, "__version__", "13.0")
                ws_major_version = int(ver_str.split('.')[0])
            except Exception:
                pass

            connect_kwargs = {
                "ping_interval": None,
                "additional_headers" if ws_major_version >= 14 else "extra_headers": headers
            }
