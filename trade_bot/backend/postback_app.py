...
async def get_calendar():
    """Реальный экономический календарь от Forex Factory"""
    from datetime import datetime, timezone, timedelta

    now = datetime.now(timezone.utc)
    now_str = now.strftime("%Y-%m-%dT%H:%M:%SZ")
    cutoff_future = (now + timedelta(hours=24)).strftime("%Y-%m-%dT%H:%M:%SZ")
    ...
    async with httpx.AsyncClient() as client:
        ...
        for url in urls:
            try:
                response = await client.get(url, timeout=8.0, headers={
                ...
                logger.warning(f"Forex Factory fetch error for {url}: {e}")
        ...
        for item in all_events:
            try:
                ...
                utc_str = event_dt_utc.strftime("%Y-%m-%dT%H:%M:%SZ")

                impact = item.get("impact", "Low")
                if impact.lower() in ("high", "3"): impact = "High"
                elif impact.lower() in ("medium", "moderate", "2"): impact = "Medium"
                else: impact = "Low"
        ...
        # Only upcoming events (next 24h, no past events)
        filtered = [e for e in normalized if now_str <= e["date"] <= cutoff_future]
        if not filtered and normalized:
            filtered = [e for e in normalized if e["date"] >= now_str][:20]

        logger.info(f"Forex Factory: {len(filtered)} upcoming events")
        return filtered  # empty [] if nothing found
