...
const COURSES = [
    {
        id: 'c0',
        icon: '📊',
        title: 'Основа трейдинга',
        desc: 'Процесс покупки и продажи активов, ликвидность, имбаланс и базовые понятия.',
...
📦 Консолидация
Это зона, где цена двигается в узком диапазоне.
...
Пробой происходит, когда цена выходит за уровень поддержки или сопротивления.`,
    ...
    },
    {
        ...
        desc: 'Процесс контроля потерь, правила управления капиталом и защита депозита.',
...
Допустим у трейдера депозит: 100$
Если риск = 2%, то максимальный убыток в одной сделке: 2$
...
2️⃣ Не открывать слишком много сделок
...
const INDICATORS = [
    {
        ...
        desc: 'Линии Боллинджера: измерение волатильности и поиск зон разворота.',
        text: `Индикатор Bollinger Bands («Линии Боллинджера») придуман техническим аналитиком Джоном Боллинджером. Впервые информация о Полосах Боллинджера была опубликована в 1987 году.<br><br>Индикатор состоит из 3-х линий, задача которых измерять текущую волатильность рынка.<br><br><b>Волны Боллинджера</b><img src="./bollinger.jpg?v=61" style="width:100%; border-radius:10px; margin:10px 0;"><br>Если говорить математическим языком, то Волны Боллинджера имеет следующую формулу:<br><br>Центральная линия (Middle Line, ML) = Moving average (SMA (CLOSE, N)),<br>Верхняя полоса (Upper Band) = ML + (D * Standard Deviation),<br>Нижняя полоса (Lower Band) = ML — (D * Standard Deviation),<br><br>Расстояние отклонения верхней и нижней линии BB от средней линии пропорционально зависит от поведения цены. Чем выше волатильность, тем сильнее отклонение и шире коридор. Чем волатильность ниже, тем отклонение меньше и уже коридор.<br><br><b>Верхние и нижние линии BB</b><img src="./bollinger_2.jpg?v=61" style="width:100%; border-radius:10px; margin:10px 0;"><br>Вы можете самостоятельно поиграться с настройками, но помните, что существенное изменение параметров сильно скажется на чувствительности индикатора. Скользящая средняя лучше всего отображает динамику поведения цены, если её строить по ценам закрытия (Close), поэтому данный параметр также не стоит менять.<br><br>Индикатор Полосы Боллинджера хорошо работает на всех таймфреймах, но не забывайте — «чем выше временной период, тем надёжнее показатели индикатора».`,
        img: null
    },
    {
        icon: '📊',
        title: 'MACD',
        desc: 'Трендовый осциллятор для определения направления и силы тренда.',
        text: `MACD – это осциллятор хотя его часто называют трендовым индикатором, или даже «трендовым осциллятором». Происходит это потому, что MACD строится на основе скользящих средних.<br><br>Сам MACD выводится в отдельном окне под графиком, и выглядит как гистограмма с вспомогательной линией. Гистограмма отображает степень расхождения двух скользящих средних. Таким образом, при сильном движении столбцы гистограммы MACD будут длинными, при флете – короткими.<br><br><b>Как выглядит индикатор MACD</b><img src="./macd.jpg?v=61" style="width:100%; border-radius:10px; margin:10px 0;"><br>Если гистограмма строится выше нулевой линии, тренд восходящий. Если столбцы выстраиваются от нулевой отметки вниз, тренд нисходящий.<br><br>Вспомогательная линия – это скользящая средняя, построенная на показаниях гистограммы MACD. Эта линия строится для получения от индикатора дополнительных сигналов.`,
        img: null
    },
    {
        icon: '🔄',
        title: 'Stochastic Oscillator',
        desc: 'Осциллятор для поиска зон перекупленности и перепроданности.',
        text: `Стохастический осциллятор устанавливается в отдельном окне под графиком цены. В классическом виде Стохастик имеет две скользящие, и со стандартными настройками этот индикатор гораздо динамичнее RSI.<br><br><b>Стохастический осциллятор</b><img src="./stochastic.jpg?v=61" style="width:100%; border-radius:10px; margin:10px 0;"><br>Быстрая линия %К выстраивается исходя из последнего значения цены, а медленная линия %D является скользящей средней относительно %К.<br><br>Благодаря наличию двух линий осциллятора трейдер получает больше вариантов сигналов, которые можно использовать для торговли.`,
        img: null
    },
    {
        icon: '📉',
        title: 'ADX',
        desc: 'Индикатор силы тренда и направленности движения.',
        text: `ADX представляет собой индикатор, образованный тремя линиями:<br><br>1. <b>+DI</b> (обычно синий) – индикатор положительного направления.<br>2. <b>-DI</b> (обычно красный) – индикатор отрицательной направленности.<br>3. <b>ADX</b> – непосредственно основная линия, показывающая силу тренда.<br><br><b>Индикатор ADX на графике</b><img src="./adx.jpg?v=61" style="width:100%; border-radius:10px; margin:10px 0;"><br>Максимальное значение ADX = 100. Достижение 50 говорит об очень сильном тренде. Нахождение в диапазоне 20-50 говорит о зарождающемся тренде, а ниже 20 – рынок находится во флете.`,
        img: null
    },
    {
        icon: '💠',
        title: 'Fractals',
        desc: 'Поиск локальных экстремумов цены по методу Билла Вильямса.',
        text: `Индикатор Фракталы по Вильямсу состоит из свечей, средняя из которых является либо самой высокой («фрактал вверх»), либо самой низкой («фрактал вниз»).<br><br><b>Индикатор Fractals</b><img src="./fractals.jpg?v=61" style="width:100%; border-radius:10px; margin:10px 0;"><br>Пользователь получает возможность четко видеть на графике экстремум цены за определенный период.<br><br>Главное условие, чтобы центральный бар был выше/ниже всех остальных. Прежде чем принимать решение, необходимо дождаться закрытия последней свечи, чтобы фигура окончательно зафиксировалась.`,
        img: null
    },
    {
        icon: '🕯️',
        title: 'RSI',
        desc: 'Индекс относительной силы: скорость и изменение цен.',
        text: `Индикатор RSI был разработан Уэллсом Уайлдером в 1978 году. Показания RSI колеблются между 0 и 100.<br><br>RSI считается перекупленным при значении выше 70, и перепроданным при значении ниже 30. Стандартная настройка — 14 периодов.<br><br><b>Индикатор RSI пример</b><img src="./rsi.png?v=61" style="width:100%; border-radius:10px; margin:10px 0;"><br>RSI сравнивает среднее движение цены. Чем больше было бычьих свечей по сравнению с медвежьими, тем больше будет значение индикатора.<br><br><b>Интерпретация RSI</b><img src="./rsi_2.png?v=61" style="width:100%; border-radius:10px; margin:10px 0;"><br>Использование RSI поможет вам добиться большей уверенности в своей торговле. Однако опасно полагать, что только из-за того, что RSI показывает перекупленность, рынок сразу развернется.`,
        img: null
    }
];
